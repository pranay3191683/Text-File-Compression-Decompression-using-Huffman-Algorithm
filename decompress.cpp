...
void Huffman::buildDecodingTree(ifstream & codeIn)
{
    ...
    while (true)
    {
        codeIn >> std::ws;  // skip whitespace/newlines

        if (codeIn.peek() == EOF) break;  // end of file

        codeIn >> ch >> code;

        // Extra protection: if code is empty or contains anything other than '0' or '1'
        if (code.empty()) {
            cerr << "*** Skipping empty code ***\n";
            continue;
        }
        ...
        for (char c : code) {
            if (c != '0' && c != '1') {
                ...
                continue;  // Skip bad lines, don't exit
            }
        }
